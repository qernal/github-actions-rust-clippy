...
jobs:
  lint:
    ...
    steps:
      ...
      - uses: qernal/github-actions-rust-clippy@v1.8.0
```
...
```bash
docker run --rm -v `pwd`:/github/workspace ghcr.io/qernal/gh-actions/rust-clippy-x86_64:v1.8.0
```
...
```bash
docker build -t ghcr.io/qernal/gh-actions/rust-clippy-x86_64:v1.8.0 -f ./Dockerfile ./
```
...
docker run --rm -e INPUT_PATH_GLOB=src/functions/*/*/ -e INPUT_THREADS=4 -e INPUT_GIT_SSH_KEY="$(cat ~/.ssh/my_key | base64 -w0)" -v `pwd`:/github/workspace ghcr.io/qernal/gh-actions/rust-clippy-x86_64:v1.8.0

# Specifiying rust version
docker run --rm -e INPUT_RUST_VERSION=1.56 -e INPUT_THREADS=4 -e INPUT_GIT_SSH_KEY="$(cat ~/.ssh/my_key | base64 -w0)" -v `pwd`:/github/workspace ghcr.io/qernal/gh-actions/rust-clippy-x86_64:v1.8.0
```